...
BASE_LOGDIR = "./logs/ResNet-2"

class Modified3DResNet18(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # load the ResNet18 model pre-trained on Kinetics
...
def train_model(task, data_path, labels_file, batch_size, lr, patience, max_epochs, resume_checkpoint=None, resume_fold=None):
    ...
    if resume_checkpoint is None:
        ...
        logdir = os.path.join(BASE_LOGDIR, f"resnet18_{task}_{timestamp}")
        ...
        # when resuming, infer logdir from the checkpoint path.
    ...
    for fold, (train_val_patient_idx, test_patient_idx) in enumerate(skf.split(unique_patients, unique_labels)):
        ...
        # if we're resuming testing for a specific fold, skip training for that fold.
        if resume_checkpoint is not None and resume_fold is not None and fold == int(resume_fold):
            print(f"Resuming testing for Fold {fold} using checkpoint {resume_checkpoint}")
        else:
        ...
        if resume_checkpoint is not None and resume_fold is not None and fold == int(resume_fold):
            best_model_path = resume_checkpoint
        else:
            best_model_path = os.path.join(logdir, f"fold_{fold}_best_model.pth")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Modified 3D ResNet18 model for death prediction')
    ...
    parser.add_argument('--data_path', type=str, default='/projects/b1038/Pulmonary/ksenkow/CLAD_serial_CT/data/6multiplied',
                        help='Path to the data directory')
    parser.add_argument('--labels_file', type=str, default='/projects/b1038/Pulmonary/ksenkow/CLAD_serial_CT/data/v2_analysis/01gather_data/mortality_metadata.csv',
